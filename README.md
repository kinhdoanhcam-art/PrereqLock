...
Tested actor: 0x5a52d040581A76e2C032542855D31480f2ea7097
...
Creates the gate, submits semantic rules and records that the condition has been met.

Actor

A distinct wallet. It alone may perform the guarded action after the edge is installed and the condition is recorded.

Observer

Reads accepted state and the immutable rule-attempt log.
...
These offline results are separate from StudioNet evidence. On the current deployment, a fresh end-to-end application flow passed on 2026-09-23 UTC:
...
The create, rule, condition and action transactions were all FINALIZED, GenVM SUCCESS and consensus Accepted, and accepted-state reads confirmed edge_installed=true, condition_met=true and action_done=true. The older K1-K5 semantic suite, negative authorization cases, replay protection and two-attempt bound remain clearly labelled as historical regression evidence for the same v1.1 source hash. Full transaction hashes and scope labels are recorded in TESTING.md.
